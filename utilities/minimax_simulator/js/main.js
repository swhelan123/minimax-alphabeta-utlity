...
function gerarExemplo() {
  ...
  var minLevels = 3; // minimum number of levels (root + at least one more)
  var maxLevels = 5; // maximum depth of the tree
  var levels = Math.floor(Math.random() * (maxLevels - minLevels + 1)) + minLevels;
  ...
  // Breadth-first generation using a simple queue:
  var queue = [];
  // root has depth `levels - 1` more levels to generate below it
  queue.push({ pk: raiz.pk, depth: levels - 1 });

  while (queue.length > 0) {
    var item = queue.shift();
    if (!item) continue;
    var parentPk = item.pk;
    var depthLeft = item.depth;

    // if no more depth, this node is a leaf
    if (depthLeft <= 0) {
      continue;
    }

    // choose how many children to create for this parent
    var nChildren = Math.floor(Math.random() * (maxChildren - minChildren + 1)) + minChildren;

    for (var i = 0; i < nChildren; i++) {
      adicionarFilho(parentPk);
      var child = nodos[nodos.length - 1];

      // If this is the last level, set a random value for the leaf
      if (depthLeft - 1 <= 0) {
        child.valor = Math.floor(Math.random() * (maxValue - minValue + 1)) + minValue;
      } else {
        // Decide probabilistically whether this child will become an internal node
        // This helps produce a variety of shapes; 0.75 means 75% chance to continue deeper
        var willExpand = Math.random() < 0.75;
        if (willExpand) {
          queue.push({ pk: child.pk, depth: depthLeft - 1 });
        } else {
          // This child becomes a leaf now
          child.valor = Math.floor(Math.random() * (maxValue - minValue + 1)) + minValue;
        }
      }
    }
  ...
  // Make sure root isn't an empty leaf; if it has no children, assign a value
  if (raiz.filhos.isEmpty()) {
    raiz.valor = Math.floor(Math.random() * (maxValue - minValue + 1)) + minValue;
  }

  // Redraw the canvas with the new random tree
  tela.draw();
}
